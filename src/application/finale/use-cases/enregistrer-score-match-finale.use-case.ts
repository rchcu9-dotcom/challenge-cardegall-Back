  ...
  async execute(
    ...
    if (!match) {
      throw new NotFoundException(
        `Match de phase finale ${matchId} introuvable`,
      );
    }
    if (scoreA === scoreB) {
    ...
    const matchMisAJour: MatchFinale = {
      ...match,
      scoreA,
      scoreB,
      statut: 'termine',
    ...
    if (
      matchMisAJour.type === 'demi_finale_a' ||
      matchMisAJour.type === 'demi_finale_b'
    ) {
      if (
        phaseFinale.finaleCardebatId === null &&
        phaseFinale.finaleLeGallId === null
      ) {
        ...
        if (
          demiFinaleA?.statut === 'termine' &&
          demiFinaleB?.statut === 'termine'
        ) {
          const { finaleCardebat, finaleLeGall } =
            this.phaseFinaleService.creerFinales(demiFinaleA, demiFinaleB);
      ...
      const finaleCardebat =
        matchMisAJour.type === 'finale_cardebat'
          ? matchMisAJour
          : autreFinaleCardebat;
      const finaleLeGall =
        matchMisAJour.type === 'finale_le_gall'
          ? matchMisAJour
          : autreFinaleLeGall;

      if (
        finaleCardebat?.statut === 'termine' &&
        finaleLeGall?.statut === 'termine'
      ) {
